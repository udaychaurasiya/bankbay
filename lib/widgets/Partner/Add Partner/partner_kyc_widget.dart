...
class _PartnerKYCScreenState extends State<PartnerKYCScreen> {
  ...
  dataSave()async{
    bool status = await controller.getCustomerDetailsNetworkApi(GetStorage().read(AppConstant.partner_cust_id));
    if(status==true){
      panCard.text = controller.customerDetails.value.data.details.custPan??"";
    }
  }
  @override
    ...
    return Container(
      ...
      child: Column(
        children: [
            ...
            child: Row(
              ...
              children:  [
                ...
                CustomButton(
                  ...
                  radius: 0,
                  onTap: ()async{
                    bool status = await controller.partnerKYCUpdateNetworkApi(GetStorage().read(AppConstant.partner_cust_id).toString().trim(),panCard.text, aadhaar.text);
                    if(status==true){
          ...
          SizedBox(height: Get.height/4.4.r),
        ],
      ),
    );
  }
