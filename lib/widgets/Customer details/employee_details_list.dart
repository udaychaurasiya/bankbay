...
class EmployeeDetailsList extends StatefulWidget {
  ...
  const EmployeeDetailsList({super.key, this.employeeId, this.pageName});
...
class _EmployeeDetailsListState extends State<EmployeeDetailsList> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text("Employee Page"),),
      body: WillPopScope(
        onWillPop: () async {
          Get.back();
          return true;
          ...
          child: SingleChildScrollView(
            child: GetBuilder<PartnerController>(builder:(controller) => Column(
              children: [
                          ...
                          children: [
                            InkWell(
                              ...
                              child: CircleAvatar(
                                ...
                                backgroundImage: NetworkImage(AppConstants.BASE_URL+controller.employeeDetails.value.data.basicDetails.docProfilePic.toString()),
                              ),
                            ),
                            SizedBox(width: 10.r),
                            Expanded(
                                      ...
                                      Expanded(
                                        child: Column(
                                          ...
                                          children: [
                                            RichText(
                                              text: TextSpan(
                                                text: "${controller.employeeDetails.value.data.basicDetails.empFname??"NA"} ${controller.employeeDetails.value.data.basicDetails.empFname??""}",
                                                style: fontSmall.copyWith(fontWeight: FontWeight.w500),
                                                children: [
                                                  TextSpan(text:controller.employeeDetails.value.data.basicDetails.eid!=null&&controller.employeeDetails.value.data.basicDetails.eid!=""?" (${controller.employeeDetails.value.data.basicDetails.eid??""})":"", style: fontSmall.copyWith(fontWeight: FontWeight.w500)),
                                                ],
                                              ),
                                              overflow: TextOverflow.ellipsis,
                                              maxLines: 1,
                                            ),
                                            // Text(controller.employeeDetails.value.data.details.custName??"NA", style: fontSmall.copyWith(fontWeight: FontWeight.w500)),
                                            Text(controller.employeeDetails.value.data.basicDetails.empPresentAddress??"NA", style: fontExtraSmall.copyWith(fontWeight: FontWeight.w400,color: AllColors.black.withOpacity(0.6)),),
                                            Text(controller.employeeDetails.value.data.basicDetails.empJoinDate??"NA", style: fontExtraSmall.copyWith(fontWeight: FontWeight.w400,color: AllColors.black.withOpacity(0.6)),),
                                          ],
                                        ),
                                      ),
                                    ],
                                      ...
                                      Container(
                                        ...
                                        child: Column(
                                          ...
                                          children: [
                                            ...
                                            Text(index==0?"${controller.employeeDetails.value.allLeadecount??0}":index==1?"${controller.employeeDetails.value.activeLeadecount??0}":"${controller.employeeDetails.value.droppedLeadecount??0}",style: fontRegular.copyWith(fontSize: 16.r,color: AllColors.black.withOpacity(0.6)),),
                                          ],
                                        ),
                                      ),
                                      Container(
                ...
                if(controller.employeeDetails.value.data.allLeade.isNotEmpty)
                  Container(
                    color: AllColors.white,
                    margin: EdgeInsets.symmetric(horizontal: 10.0.r, vertical: 10.r),
                    child: ListView.builder(
                      itemCount: controller.employeeDetails.value.data.allLeade.length,
                      physics: const NeverScrollableScrollPhysics(),
                      shrinkWrap: true,
                      itemBuilder: (context, index) {
                        var allLeadsData = controller.employeeDetails.value.data.allLeade[index];
                        return Container(
                ...
                if(controller.employeeDetails.value.data.allLeade.isEmpty)
