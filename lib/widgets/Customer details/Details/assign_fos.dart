...
class AssignFos extends StatefulWidget {
  final String? leadId;
  final String? apiLeadId;
  final String? leadTransferName;
  final String? pageName;
  const AssignFos({Key? key, this.leadId, this.apiLeadId, this.leadTransferName, this.pageName}) : super(key: key);

  @override
  State<AssignFos> createState() => _AssignFosState();
}
...
class _AssignFosState extends State<AssignFos> {
  @override
  Widget build(BuildContext context) {
    ...
    return Obx(() => Column(
      ...
      children: [
        UnderLineTextField(
          controller: controller.fosCheckID,
          ...
          suffixIcon: TextButton(
            ...
            onPressed: (){
              controller.checkOwnerExistFosNetworkApi(
                  findValue.value=="Pan"?"pan":findValue.value=="phone"?"phone":
                  findValue.value=="UID"?"UID":"",  controller.fosCheckID.text);
              controller.check_owner_exist_fos.refresh();
            },
        ...
        Column(
          ...
          children: [
            ...
            controller.check_owner_exist_fos.value.data.isNotEmpty?
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text("${controller.check_owner_exist_fos.value.data[0].empFname??"NA"} ${controller.check_owner_exist_fos.value.data[0].empLname} (${controller.check_owner_exist_fos.value.data[0].eid??"NA"})" ,style: fontMedium.copyWith(color: AllColors.black.withOpacity(0.6), fontSize: 13.r,),),
                SizedBox(height: 4.r),
                Text("${controller.check_owner_exist_fos.value.data[0].empPresentAddress??"NA"} " ,style: fontMedium.copyWith(color: AllColors.black, fontSize: 13.r,),),
                SizedBox(height: 4.r),
                RichText(
                  text: TextSpan(
                    text: 'Sup ID : ',
                    ...
                    children: [
                      TextSpan(text: "${controller.check_owner_exist_fos.value.data[0].supId??"NA"} (${controller.check_owner_exist_fos.value.data[0].supId} )",style: fontMedium.copyWith(color: AllColors.black, fontSize: 13.r,),),
                    ],
                  ),
                ),
                SizedBox(height: 4.r),
                RichText(
                  text: TextSpan(
                    ...
                    style: fontMedium.copyWith(color: AllColors.grey),
                    children: [
                      TextSpan(text: "${controller.check_owner_exist_fos.value.data[0].empFunction??"NA"} ",style: fontMedium.copyWith(color: AllColors.black, fontSize: 13.r,),),
                    ],
                  ),
                )
              ],
        ...
        //     onTap: ()async{
        //       if(controller.check_owner_exist_fos.value.data.isNotEmpty &&
        //           controller.check_owner_exist_fos.value.data!="null"){
        //         bool status = await controller.updateLeadAssignFosNetworkApi(controller.getLeadDetails.value.data.leadId,
        //           controller.check_owner_exist_fos.value.data[0].eid, controller.getLeadDetails.value.data.lockedBy.toString(),
        //         );
        //         if(status==true) {
