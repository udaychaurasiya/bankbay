...
import 'package:bank_bay/widgets/Apply%20Loan/Bussiness%20Loan/business_loan_document.dart';
...
class BusinessLoanForm extends StatefulWidget {
  ...
  const BusinessLoanForm({super.key, required this.loanType});
...
class _BusinessLoanFormState extends State<BusinessLoanForm> {
  ...
  Widget build(BuildContext context) {
    print("wjbbsx  ${widget.loanType}");
    controller.employeeRefreshList();
    return Scaffold(
      appBar: AppBar(title: const Text("Business Loan"),),
      body: Container(
        width: Get.width,
                      ...
                      onTap: () async{
                        if(loanActive.value == 1){
                          if ((formKeyLoanRequirment.currentState != null && formKeyLoanRequirment.currentState!.validate())&&
                            ...
                            bool status = await controller.customerLoanLeadNetworkApi("BL", partnerController.getLeadDetails.value.data.leadId??"", widget.loanType);
                            if(status==true){
                              Get.to(()=> const BusinessLoanDocument());
                            }
                          }
                        }else{
                          if ((formKeyLoanRequirment.currentState != null && formKeyLoanRequirment.currentState!.validate())){
