...
import 'personal_loan_document.dart';

class PersonalLoanForm extends StatefulWidget {
  final String loanType;
  const PersonalLoanForm({super.key, required this.loanType});
                        ...
                        onTap: () async{
                          if(loanActive.value == 1){
                            if ((formKeyLoanRequirment.currentState != null && formKeyLoanRequirment.currentState!.validate())&&
                              ...
                              bool status = await controller.customerLoanLeadNetworkApi("PL", partnerController.getLeadDetails.value.data.leadId??"", widget.loanType);
                              if(status==true){
                                Get.to(()=> const PersonalLoanDocument());
                              }
                            }
                          }else{
                            if ((formKeyLoanRequirment.currentState != null && formKeyLoanRequirment.currentState!.validate())){
