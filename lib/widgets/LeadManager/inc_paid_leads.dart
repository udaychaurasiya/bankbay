...
class IncPaidLeads extends StatefulWidget {
  const IncPaidLeads({super.key});

  @override
  State<IncPaidLeads> createState() => _IncPaidLeadsState();
}

class _IncPaidLeadsState extends State<IncPaidLeads> {
                              ...
                              detailsTap: ()async{
                                ...
                                if(checkStatus==true){
                                  if(controller.allLeadsDataModel.value.data.newLeade[index].leadLockStatus=="false") {
                                    ...
                                    if(status==true) {
                                      Get.to(()=> CustomerDetails(leadId: "${data.id??""}",customerId: data.custId??"",value: "incDetails",apiLeadId: data.leadId, pageName: "incPaidLeads",));
                                      controller.allLeadsDataNetworkApi("","","",partnerController.loanTypeDecode(selectLoanType.value.toString()),"","","",controller.leadsPageName.value,controller.filterTypeSelected.value,partnerController.leadStatusNameFilter(controller.selectEmployeePartnerValue.value));
                                    }
                                  }else{
                                        ...
                                        Padding(
                                          ...
                                          child: Row(
                                            ...
                                            children: [
                                              ...
                                              Row(
                                                children: [
                                                  Icon(Icons.wallet_outlined, size: 16.r, color: AllColors.green.withOpacity(0.7),),
                                                  SizedBox(width: 5.r),
                                                  Text("Inc Paid", style: fontExtraSmall.copyWith(color: AllColors.black.withOpacity(0.7),letterSpacing: 0, wordSpacing: 0),),
                                                ],
                                              ),
                                            ],
                                          ),
