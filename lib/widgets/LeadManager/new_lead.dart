...
class _NewLeadScreenState extends State<NewLeadScreen> {
  ...
  @override
  void initState() {
    controller.allLeadsDataNetworkApi("","","",partnerController.loanTypeDecode(controller.selectLoanType.value.toString()),"","","",partnerController.leadStatusNameFilter(controller.leadsPageName.value.toString()),partnerController.sourceFilterSendValue(controller.filterTypeSelected.value.toString()),partnerController.leadStatusNameFilter(controller.selectEmployeePartnerValue.value));
    super.initState();
  }
                ...
                children: [
                  ...
                  InkWell(
                    onTap: () {
                      showMenu(
                        context: context,
                        position: RelativeRect.fromLTRB(Get.width/3.8, Get.width/2.8, Get.width/2, 0), // Adjust the position of the popup menu
                        items: <PopupMenuEntry<String>>[
                          PopupMenuItem(
                            value: 'All Source'.tr,
                            child: SizedBox(
          ...
          Container(
            ...
            child: ListView(
              ...
              children: [
                ...
                SizedBox(width: 15.r),
                if(controller.filterTypeSelected.value=="Customer")
                buildRadioButtonEMP('Unassigned', "Unassigned"),
                if(controller.filterTypeSelected.value=="Customer")
                SizedBox(width: 15.r),
                if(controller.filterTypeSelected.value=="Customer")
                buildRadioButtonEMP("Assigned", "Assigned"),
                if(controller.filterTypeSelected.value=="Customer")
                // SizedBox(width: 15.r),
                // buildRadioButtonEMP("New", "New"),
                SizedBox(width: 15.r),
                buildRadioButtonEMP("Rejected", "Rejected"),
                SizedBox(width: 15.r),
                buildRadioButtonEMP("Draft", "Draft"),
              ],
            ),
          ),
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        InkWell(
                          onTap: ()async{
                            bool status = await controller.allLeadsDataNetworkApi("","","",partnerController.loanTypeDecode(controller.selectLoanType.value.toString()),"","","",partnerController.leadStatusNameFilter(controller.leadsPageName.value.toString()),partnerController.sourceFilterSendValue(controller.filterTypeSelected.value.toString()),partnerController.leadStatusNameFilter(controller.selectEmployeePartnerValue.value));
                            if(status==true){
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    GetBuilder<LoginController>(builder: (controller) {
                        return controller.new_lead_loader.value?
                        ListView.builder(
                            ...
                            itemBuilder: (context, index){
                              ...
                              return CustomLeadsDetails(
                                ...
                                index: isLastItem,
                                detailsTap: ()async{
                                  bool checkStatus = await controller.allLeadsDataNetworkApi("","","",partnerController.loanTypeDecode(controller.selectLoanType.value.toString()),"","","",partnerController.leadStatusNameFilter(controller.leadsPageName.value.toString()),partnerController.sourceFilterSendValue(controller.filterTypeSelected.value.toString()),partnerController.leadStatusNameFilter(controller.selectEmployeePartnerValue.value));
                                  if(checkStatus==true){
                                    if(controller.allLeadsDataModel.value.data.newLeade[index].leadLockStatus=="false") {
                                      ...
                                      if(status==true) {
                                        Get.to(()=> CustomerDetails(leadId: "${data.id??""}",customerId: data.custId??"",value: "leadDetails",apiLeadId: data.leadId,pageName: "newLead",leadTransferName: "unassignedTransferLead"));
                                        controller.allLeadsDataNetworkApi("","","",partnerController.loanTypeDecode(controller.selectLoanType.value.toString()),"","","",partnerController.leadStatusNameFilter(controller.leadsPageName.value.toString()),partnerController.sourceFilterSendValue(controller.filterTypeSelected.value.toString()),partnerController.leadStatusNameFilter(controller.selectEmployeePartnerValue.value));
                                      }
                                    }else{
                                ...
                                nameTap: ()=> Get.to(()=> CustomerLoanListDetails(leadId: "${data.id??""}",customerId: data.custId??"" ,apiLeadId: data.leadId,leadTransferName: "unassignedTransferLead",)),
                                    ...
                                    if(value == "Transfer Lead"){
                                      ...
                                      if(checkStatus==true){
                                        if(controller.allLeadsDataModel.value.data.newLeade[index].leadLockStatus=="false") {
                                          bool status = await permissionController.lockUnlockLeadNetworkApi(controller.allLeadsDataModel.value.data.newLeade[index].leadId, 'true');
                                          if(status==true) {
                                            Get.to(()=> CustomerDetails(leadId: "${data.id??""}",customerId: data.custId??"",value: "leadDetails",apiLeadId: data.leadId,pageName: "newLead",leadTransferName: "transferLead",));
                                            controller.allLeadsDataNetworkApi("","","",partnerController.loanTypeDecode(controller.selectLoanType.value.toString()),"","","",partnerController.leadStatusNameFilter(controller.leadsPageName.value.toString()),partnerController.sourceFilterSendValue(controller.filterTypeSelected.value.toString()),partnerController.leadStatusNameFilter(controller.selectEmployeePartnerValue.value));
                                          }
                                        }else{
                                          BaseController().errorSnack("Already details View on ${controller.allLeadsDataModel.value.data.newLeade[index].lockedBy.toString()}");
                                      ...
                                      bool checkStatus = await controller.allLeadsDataNetworkApi("","","",partnerController.loanTypeDecode(controller.selectLoanType.value.toString()),"","","",partnerController.leadStatusNameFilter(controller.leadsPageName.value),partnerController.sourceFilterSendValue(controller.filterTypeSelected.value.toString()),partnerController.leadStatusNameFilter(controller.selectEmployeePartnerValue.value));
                                      if(checkStatus==true){
                                        if(controller.allLeadsDataModel.value.data.newLeade[index].leadLockStatus=="false") {
                                          bool status = await permissionController.lockUnlockLeadNetworkApi(controller.allLeadsDataModel.value.data.newLeade[index].leadId, 'true');
                                          if(status==true) {
                    ...
                    if(controller.allLeadsDataModel.value.message.toString()=="Data Not Found.")
                      Container(
        ...
        SizedBox(
          ...
          child: Radio(
            ...
            onChanged: (selectedValue) {
              ...
              print("nsnnkj ${controller.leadsPageName.value}");
              controller.allLeadsDataNext = 10;
              if(selectedValue=="All"){
                controller.allLeadsDataNetworkApi("","","",partnerController.loanTypeDecode(controller.selectLoanType.value.toString()),"","","", partnerController.leadStatusNameFilter(selectedValue.toString()),partnerController.sourceFilterSendValue(controller.filterTypeSelected.value.toString()),partnerController.leadStatusNameFilter(controller.selectEmployeePartnerValue.value));
                      ...
                      children: [
                        ...
                        CustomButton(
                          ...
                          onTap: () async {
                            ...
                            // controller.allLeadsDataNetworkApi("","","",controller.customerPan.text, controller.customerPhone.text, controller.leadStartDate.text, controller.leadEndDate.text,controller.newLeadRadio.=="All"?"":,controller.newLeadRadio.value.toLowerCase(),"");
                            controller.loanType.value="All Loan";
                            controller.customerPan.clear();
                            controller.customerPhone.clear();
                            controller.leadStartDate.clear();
