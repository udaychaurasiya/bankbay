...
class HoldLeads extends StatefulWidget {
  const HoldLeads({super.key});

  @override
  State<HoldLeads> createState() => _HoldLeadsState();
}

class _HoldLeadsState extends State<HoldLeads> {
        ...
        children: [
          ...
          Expanded(
            child: SingleChildScrollView(
              controller: controller.holdLeadController,
              child: Container(
                              ...
                              detailsTap: ()async{
                                ...
                                if(checkStatus==true){
                                  if(controller.allLeadsDataModel.value.data.newLeade[index].leadLockStatus=="false") {
                                    ...
                                    if(status==true) {
                                      Get.to(()=> CustomerDetails(leadId: "${data.id??""}",customerId: data.custId??"",value: "leadDetails",apiLeadId: data.leadId, pageName: "holdLeads",));
                                      controller.allLeadsDataNetworkApi("","","",partnerController.loanTypeDecode(selectLoanType.value.toString()),"","","",controller.leadsPageName.value,controller.filterTypeSelected.value,partnerController.leadStatusNameFilter(controller.selectEmployeePartnerValue.value));
                                    }
                                  }else{
                                  ...
                                  if(value == "Transfer Lead"){
                                    ...
                                    if(checkStatus==true){
                                      if(controller.allLeadsDataModel.value.data.newLeade[index].leadLockStatus=="false") {
                                        bool status = await permissionController.lockUnlockLeadNetworkApi(controller.allLeadsDataModel.value.data.newLeade[index].leadId, "true");
                                        if(status==true) {
                                          Get.to(()=> CustomerDetails(leadId: "${data.id??""}",customerId: data.custId??"",value: "leadDetails",apiLeadId: data.leadId, pageName: "holdLeads",leadTransferName: "transferLead",));
                                          controller.allLeadsDataNetworkApi("","","",partnerController.loanTypeDecode(selectLoanType.value.toString()),"","","",controller.leadsPageName.value,controller.filterTypeSelected.value,partnerController.leadStatusNameFilter(controller.selectEmployeePartnerValue.value));
                                        }
                                      }else{
                                        BaseController().errorSnack("Already details View on ${controller.allLeadsDataModel.value.data.newLeade[index].lockedBy.toString()}");
