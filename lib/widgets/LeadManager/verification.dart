    ...
    return Scaffold(
      ...
      body: Obx(()=> Column(
        children: [
          Container(
            height: 45.r,
            padding: EdgeInsets.symmetric(horizontal: 8.r, vertical:10.0.r),
            child: ListView(
              ...
              children: [
                ...
                SizedBox(width: controller.filterTypeSelected.value=="All Source" || controller.filterTypeSelected.value=="Customer"?40.w:15.w),
                if(GetStorage().read(AppConstant.cust_id).toString().startsWith("E10"))
                InkWell(
                  onTap: () {
                    showMenu(
                      context: context,
                      position: RelativeRect.fromSize(Rect.fromCircle(center: const Offset(76, 165), radius: 5), const Size(150, 300)),
                      items: <PopupMenuEntry<String>>[
                        PopupMenuItem(
                          value: 'All Source'.tr,
                          child: SizedBox(
                    ...
                    child: InkWell(
                      onTap: () {
                        showMenu(
                          ...
                          print(controller.selectEmployeePartnerValue.value);
                          controller.verificationNetworkApi("","","",partnerController.loanTypeDecode(value.toString()),"","","",partnerController.leadStatusNameFilter(controller.leadsPageName.value),partnerController.sourceFilterSendValue(controller.filterTypeSelected.value),partnerController.leadStatusNameFilter(controller.selectEmployeePartnerValue.value));
                        });
                      },
                      child: Row(
                ...
                SizedBox(width: controller.filterTypeSelected.value=="All Source" || controller.filterTypeSelected.value=="Customer"?40.w:10.w),
                Row(
          ...
          Container(
            ...
            child: ListView(
              ...
              children: [
                ...
                SizedBox(width: 15.r),
                buildRadioButtonEMP('Pending Verification', "Pending Verification"),
                SizedBox(width: 15.r),
                buildRadioButtonEMP('Verification Validation', "Verification Validation"),
                SizedBox(width: 15.r),
                buildRadioButtonEMP("Validated", "Validated"),
                SizedBox(width: 15.r),
                buildRadioButtonEMP("Hold Validation", "Hold Validation"),
                SizedBox(width: 15.r),
                buildRadioButtonEMP("Hold", "Hold"),
                SizedBox(width: 15.r),
                      ...
                      children: [
                        WeeklyDatePicker(
                          ...
                          changeDay: (value)async{
                            ...
                            await controller.verificationNetworkApi(DateFormat('yyy-MM-dd').format(value).toString(),DateFormat('yyy-MM').format(value),"",partnerController.loanTypeDecode(value.toString()),"","","",partnerController.leadStatusNameFilter(controller.leadsPageName.value),partnerController.sourceFilterSendValue(controller.filterTypeSelected.value),partnerController.leadStatusNameFilter(controller.selectEmployeePartnerValue.value));
                          },
                        ...
                        InkWell(
                          onTap: ()async{
                            bool status = await controller.verificationNetworkApi("","","",partnerController.loanTypeDecode(controller.selectLoanType.value.toString()),"","","",partnerController.leadStatusNameFilter(controller.leadsPageName.value),partnerController.sourceFilterSendValue(controller.filterTypeSelected.value),partnerController.leadStatusNameFilter(controller.selectEmployeePartnerValue.value));
                            if(status==true){
          ...
          Expanded(
            child: SingleChildScrollView(
              controller: controller.verificationScroller,
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    GetBuilder<LoginController>(builder:(controller) {
                      return controller.verification_loader.value?
                      ListView.builder(
                          itemCount: controller.verificationDataModel.value.data.newLeade.length,
                          shrinkWrap: true,
                          physics: const NeverScrollableScrollPhysics(),
                          padding: EdgeInsets.only(bottom: 5.r),
                          itemBuilder: (context, index){
                            var data = controller.verificationDataModel.value.data.newLeade[index];
                            bool isLastItem = index == controller.verificationDataModel.value.data.newLeade.length - 1;
                            return CustomLeadsDetails(
                              ...
                              index: isLastItem,
                              detailsTap: ()async{
                                bool checkStatus = await controller.verificationNetworkApi("","","",partnerController.loanTypeDecode(controller.selectLoanType.value.toString()),"","","",partnerController.leadStatusNameFilter(controller.leadsPageName.value),partnerController.sourceFilterSendValue(controller.filterTypeSelected.value),partnerController.leadStatusNameFilter(controller.selectEmployeePartnerValue.value));
                                if(checkStatus==true){
                                  if(controller.verificationDataModel.value.data.newLeade[index].leadLockStatus=="false") {
                                    bool status = await permissionController.lockUnlockLeadNetworkApi(controller.verificationDataModel.value.data.newLeade[index].leadId, "true");
                                    if(status==true) {
                                      Get.to(()=> CustomerDetails(leadId: "${data.id??""}",customerId: data.custId??"",value: "leadDetails",apiLeadId: data.leadId, pageName: "verificationLead",));
                                      controller.verificationNetworkApi("","","",partnerController.loanTypeDecode(controller.selectLoanType.value.toString()),"","","",partnerController.leadStatusNameFilter(controller.leadsPageName.value),partnerController.sourceFilterSendValue(controller.filterTypeSelected.value),partnerController.leadStatusNameFilter(controller.selectEmployeePartnerValue.value));
                                    }
                                  }else{
                                    BaseController().errorSnack("Already details View on ${controller.verificationDataModel.value.data.newLeade[index].lockedBy.toString()}");
                                    controller.verificationNetworkApi("","","",partnerController.loanTypeDecode(controller.selectLoanType.value.toString()),"","","",partnerController.leadStatusNameFilter(controller.leadsPageName.value),partnerController.sourceFilterSendValue(controller.filterTypeSelected.value),partnerController.leadStatusNameFilter(controller.selectEmployeePartnerValue.value));
                                  }
                                }
                              },
                                  ...
                                  if(value == "Transfer Lead"){
                                    ...
                                    if(checkStatus==true){
                                      if(controller.verificationDataModel.value.data.newLeade[index].leadLockStatus=="false") {
                                        bool status = await permissionController.lockUnlockLeadNetworkApi(controller.verificationDataModel.value.data.newLeade[index].leadId, "true");
                                        if(status==true) {
                                          Get.to(()=> CustomerDetails(leadId: "${data.id??""}",customerId: data.custId??"",value: "leadDetails",apiLeadId: data.leadId, pageName: "verificationLead",leadTransferName: "transferLead",));
                                          controller.verificationNetworkApi("","","",partnerController.loanTypeDecode(controller.selectLoanType.value.toString()),"","","",partnerController.leadStatusNameFilter(controller.leadsPageName.value),partnerController.sourceFilterSendValue(controller.filterTypeSelected.value),partnerController.leadStatusNameFilter(controller.selectEmployeePartnerValue.value));
                                        }
                                      }else{
                                        BaseController().errorSnack("Already details View on ${controller.verificationDataModel.value.data.newLeade[index].lockedBy.toString()}");
                    ...
                    if(controller.verificationDataModel.value.message.toString()=="Data Not Found.")
                      Container(
                          alignment: Alignment.center,
                          margin: EdgeInsets.only(bottom: 10.r),
                          child: Text("Data not found !!",style: fontRegular.copyWith(letterSpacing: 0,color: AllColors.black.withOpacity(0.7)),)
                      ),
                    if(controller.isLoadMore.value) const LoadingMoreData(),
                  ],
                ),
              ),
            ),
        ...
        SizedBox(
          ...
          child: Radio(
            ...
            onChanged: (selectedValue) {
              ...
              print(controller.leadsPageName.value);
              controller.verificationNetworkApi("","","",partnerController.loanTypeDecode(controller.selectLoanType.value),"","","",partnerController.leadStatusNameFilter(selectedValue.toString()),partnerController.sourceFilterSendValue(controller.filterTypeSelected.value),partnerController.leadStatusNameFilter(controller.selectEmployeePartnerValue.value));
            },
          ),
        ),
        SizedBox(width: 4.r),
                      ...
                      children: [
                        ...
                        CustomButton(
                          ...
                          onTap: () async {
                            ...
                            controller.verificationNetworkApi("","",controller.loanType.value, controller.customerPan.text, controller.customerPhone.text, controller.leadStartDate.text, controller.leadEndDate.text,partnerController.leadStatusNameFilter(controller.leadsPageName.value),partnerController.sourceFilterSendValue(controller.filterTypeSelected.value),partnerController.leadStatusNameFilter(controller.selectEmployeePartnerValue.value));
                            controller.loanType.value="All Loan";
                            controller.customerPan.clear();
                            controller.customerPhone.clear();
                            controller.leadStartDate.clear();
