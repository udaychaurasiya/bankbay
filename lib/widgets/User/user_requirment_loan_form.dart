...
class UserRequirementLoanForm extends StatefulWidget {
  const UserRequirementLoanForm({super.key});

  @override
  State<UserRequirementLoanForm> createState() => _UserRequirementLoanFormState();
}

var controller = Get.find<LoginController>();
  var partnerController = Get.find<PartnerController>();
class _UserRequirementLoanFormState extends State<UserRequirementLoanForm> {
  ...
  Widget build(BuildContext context) {
    // controller.companyType.value="Private Limited";
    controller.employeeRefreshList();
    return Container(
      width: Get.width,
              ...
              Row(
                ...
                children: [
                  LightCustomButton(
                    ...
                    onTap: (){
                      indexValue.value = 1;
                      controller.userFieldList.clear();
                      controller.userFieldList.value.add({
                        "loan_type": null,
                        "tenure": null,
                        "month": null,
                    ...
                    onTap: () async{
                      if(loanActive.value == 1){
                        if ((formKeyLoanRequirment.currentState != null && formKeyLoanRequirment.currentState!.validate())&&
                            ...
                            bool status = await controller.customerLoanLeadNetworkApi(sliderValue.value==0?"PL":sliderValue.value==1?"BL":sliderValue.value==2?"SEL":"Pl", partnerController.getLeadDetails.value.data.leadId??"","myLoan");
                            if(status==true){
                              indexValue.value = 6;
                            }
                        }
